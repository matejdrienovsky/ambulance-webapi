...
$env:AMBULANCE_API_MONGODB_DATABASE = "xdrienovskym2-ambulance-wl"
...
switch ($command) {
    ...
    "docker" {
        docker build -t matejdrienovsky/ambulance-wl-webapi:local-build -f ${ProjectRoot}/build/docker/Dockerfile ${ProjectRoot}
    }
    default {
