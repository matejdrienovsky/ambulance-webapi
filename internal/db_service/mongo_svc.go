...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.DbName == "" {
		svc.DbName = enviro("AMBULANCE_API_MONGODB_DATABASE", "xdrienovskym2-ambulance-wl")
	}
	...
	if svc.Timeout == 0 {
		...
		if parsedSeconds, err := strconv.Atoi(seconds); err == nil {
			svc.Timeout = time.Duration(parsedSeconds) * time.Second
		} else {
